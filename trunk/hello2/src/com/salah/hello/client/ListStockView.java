...
public class ListStockView implements IsWidget, EntryPoint {

	/**
	 * Create a remote service proxy to talk to the server-side Greeting
	 * service.
	 */
	private final MyStockServiceAsync serviceRpc = GWT.create(MyStockService.class);
	...
	public Widget asWidget() {
		if (root == null) {
			...
			// Double>(props.change(), 100, "Change");
			...
			ColumnConfig<Stock, Date> lastTransCol = new ColumnConfig<Stock, Date>(props.updatedDate(), 100, "Last Updated");
			...
			ListStore<Stock> store = getListStcok();
	...
	private ListStore<Stock> getListStcok() {
		final ListStore<Stock> store = new ListStore<Stock>(props.key());

		serviceRpc.getListStock(new AsyncCallback<List<Stock>>() {
			@Override
