...
var ihm = Class.create({
	...
	init: function(tableau_langage_sujet_difficulte_disponible,_urlPictoSuppr){
		this.tabDispo  = tableau_langage_sujet_difficulte_disponible;
		this.urlPictoSuppr = _urlPictoSuppr;
	...
	ajoutePanier: function(entreeAAjouter){
		...
		if(indice > -1 && this.isIn(entreeAAjouter,this.tabChoisi) == -1){
			...
			effect("transfer", {to: "#divPanier",className: "ui-effects-transfer"}, 200, function(){ihm.refresh();});
		}else{
	...
	refreshPanier:function(){
		...
		$(this.tabChoisi).each(function(index,value){
			...
			var imgSuppr = ihm.createImg(ihm.urlPictoSuppr);
